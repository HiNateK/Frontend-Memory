...
DROP FUNCTION IF EXISTS track_trial_signup();

-- Create trigger function to track trial signups
CREATE FUNCTION track_trial_signup()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Drop existing function if it exists
DROP FUNCTION IF EXISTS create_trial_customer(text, text, text, uuid);

-- Create trial customer function with jsonb return type
CREATE FUNCTION create_trial_customer(
  p_email text,
  p_first_name text,
  p_last_name text,
  p_user_id uuid
